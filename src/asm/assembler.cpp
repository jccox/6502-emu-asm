		...
		for(j = 0; j < (int) strlen(buf); ++j)
		  if(buf[j] == '\''){
				  ...
				  strcpy(buf + j, num_buf);
			          ...
			          sprintf(errormsg, "%s has a bad ASCII character.", buf);
				  printerror(errormsg, i);
				  exit(1);
			  }
		  }
	  ...
	  lineloc = linelocb;
		...
		if(strcmp(current_word, ".ENDMACRO") == 0){
			...
			if(current_word == NULL){
		                sprintf(errormsg, "%s is not a file name.", current_word);
				printerror(errormsg, linecount);
				exit(1);
			}
			//Check for proper format of the argument.
			if(current_word[0] != '\"' || current_word[strlen(current_word) - 1] != '\"'){
			        sprintf(errormsg, "%s must be enclosed in quotes; single for internal, double for external.", current_word);
				printerror(errormsg, linecount);
				exit(1);
			}
			...
			if(tmp_in == NULL){
			        sprintf(errormsg, "%s does not open.", current_word);
				printerror(errormsg, linecount);
				exit(1);
			}
		...
		//Keep track of labels.
			...
			if(hsearch(entrybuf, FIND) != NULL){
				...
				bsearch(current_word, directives, dircard, sizeof(char*), scmp) != NULL){
			        ...
			        sprintf(errormsg, "%s is an illegal label name", current_word);
				printerror(errormsg, linecount);
				exit(1);
			}
		...
		//Check for infix directives acting as prefix directives.  
		//This is required because they are the only preprocessor directives.
		else if(strcmp(current_word, ".EQU") == 0 || strcmp(current_word, ".MACRO") == 0){
			sprintf(errormsg, "%s is not a prefix directive.  Please give a prefix argument.\n", current_word);
			printerror(errormsg, linecount);
			exit(1);
		}
		...
		//directives.
			...
			if(current_word == NULL){
			        sprintf(errormsg, "%s is an unknown directive.\n", entrybuf.key);
				printerror(errormsg, linecount);
				exit(1);
			}
			...
			if(strcmp(current_word, ".EQU") == 0){
				current_word = strtok(NULL, " ");
				if(current_word == NULL){
					printerror("No postfix operand was given for the infix operator .EQU.  Please give one", linecount);
					exit(1);
				}
				if(current_word[0] != '$'){
						printerror(".EQU only accepts a hex value ($XXXX or $XX) as an argument.", linecount);
						exit(1);
				}
				...
				sprintf(errormsg, "%s is not a preprocessor infix directive.  Please give a proper directive.\n", current_word);
				printerror(errormsg, linecount);
				exit(1);
			}
		}
		...
		/*If it is an instruction, increment the location counter.*/
			...
			if((enterreturn = hsearch(entrybuf, FIND)) != NULL){
				...
				if(((struct subword *) (enterreturn->data))->type == LABEL_T){
				  printf("2%s %s\n", (((char *) (enterreturn->key))), (((struct subword *) (enterreturn->data))->value));
					current_location += 2;
					program_size += 2;
				}
					...
					printerror("Macros are not allowed as values!\n", linecount);
					exit(1);
				}
			}
			else{
			...
			case ORG:
				if(current_word[0] != '$'){
					printerror(".ORG expects a hexadecimal address (e.g. $XXXX).\n", linecount);
					exit(1);
				}
				...
				if(current_location >= 0x10000)
					...
					if(current_location < 0){
						printerror("Negative addresses do not exist.\n", linecount);
						exit(1);
					}
					else{
		...
		//Search and give the index of the mnemonic to searchloc.
			...
			if(current_word == NULL)
				addr_mode = IMPL;
			        ...
			        /*Replace a label with its value.*/
			        if((tokenword = strchr(current_word, ')'))){
